...
.read <- function(hdr) {
    ...
    .dat <- lapply(.files, .fread, header=TRUE) %>% 
        do.call(what=rbind) %>%
...
plot.power <- function(.dt) {
    ...
    .sum <- .dt[!is.na(power),
                .(y = mean(power), y.se = sd(power)/sqrt(.N)),
...
.rename.v2 <- function(.sum) {

    .method <- c("cocoa","mu","avg","tot")
    .lab <- c("CoCoA", "mu", "average", "total")
    .sum[, method := factor(method, .method, .lab)]
...
plot.f1.v2 <- function(.dt) {
    ...
    .aes <- aes(x = p1, y = y, ymin = y - y.se, ymax = y + y.se, colour = method, lty = method)
    ...
    .ggplot(.sum, .aes) +
        ...
        scale_colour_manual(values = c("red", "gray30", "#000099", "#009900"))
}
```
...
## CoCoA improves statistical power - round 2

```{r Fig_Sim_F1_round2, fig.width=6, fig.height = 3}
plt <- plot.f1.v2(read.f1("sim_v2_N40_M50"))
print(plt)
.file <- fig.dir %&% "/Fig_Sim_F1_round2.pdf"
...
```{r Fig_Sim_F1_round2_100, fig.width=6, fig.height = 3}
plt <- plot.f1.v2(read.f1("sim_v2_N40_M100"))
print(plt)
.file <- fig.dir %&% "/Fig_Sim_F1_round2_100.pdf"
.gg.save(.file, plot=plt, width=6, height=3)
```

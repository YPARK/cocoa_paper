...
col.lty.df <- 
    tibble(
        method = c("cocoa", "cocoa.pc.safe", "cocoa.pc.full", "tot", "tot.pc.safe", "tot.pc.full", "avg", "cf"),
        label = c("CoCoA", "CoCoA + safe PC", "CoCoA + 10 PC",
                  "Total", "Tot + safe PC", "Tot + 10 PC",
                  "Average", "confounder"),
        lty = c(1, 2, 3, 1, 2, 3, 1, 1),
        pch = c(19, 21, 32, 17, 24, 32, 4, 3),
        col = c("magenta", "magenta", "magenta", "#009900", "#009900", "#009900", "#00FF00", "#000099")
    )
...
for(ss in c(1, 2, 5)) {
    ...
    .dt <- result.dt[S==ss & B == (5- ss) & N == 40 & M == 50]
...
```{r}
.take.roc <- function(.dt, .method = c("cocoa", "tot", "avg", "cf")) {
...
.dt <- result.dt[B==5 & N == 40 & M == 50] %>% .take.roc
...
```{r}
.take.power <- function(.dt, .method = c("cocoa.pc.safe","cocoa","tot", "tot.pc.safe","tot.pc.full", "cf")) {
...
for(ss in c(0, 1, 2, 5)){
    .dt <- result.dt[S == ss & B == (5-ss)] %>%  .take.power

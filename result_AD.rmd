...
n.fwer01.genes <- cocoa.dt[fwer < .01, .(gene)] %>% unique %>% nrow
...
n.fwer01.ct.genes <- cocoa.dt[fwer < .01, .(n=length(unique(gene))), by=.(ct)] %>%
    mutate(str = num.int(n) %&% " found in " %&% ct) %>%
    arrange(factor(ct, rev(.marker.order$ro)))
```

### A table for genes significant at FWER 1%

```{r include = FALSE}
dir.create("Tab/AD/", recursive=TRUE, showWarnings=FALSE)
fwer01.genes <- cocoa.dt[fwer < .01, .(gene)] %>% unique
out.dt <- left_join(fwer01.genes, cocoa.dt, by = "gene") %>% as.data.table
fwrite(out.dt, file="Tab/AD/fwer.txt.gz", row.names=FALSE, col.names=TRUE, sep="\t")
```

### What if cell types were combined? Cell-type-sorted pseudo-bulk analysis improves statistical power

```{r include = FALSE}
bulk.dt <-
    fread("result/combined/stat/1.stat.gz", header=TRUE) %>%
    mutate(ct = "bulk") %>%
    as.data.table

.count.discovery <- function(.method) {
    .cutoff <- 10^(seq(-8, -2, by = .25))

    .dt.1 <- bulk.dt[method == .method, .(gene, wilcox.pv)]
    .dt.1[, ct := "bulk"]
    .dt.2 <- glob.dt[method == .method, .(gene, ct, wilcox.pv)]

    .dt <- rbind(.dt.1, .dt.2)
    .dt <- .dt[order(.dt$wilcox.pv)]
    .dt[ct == "bulk", ct.vs.bulk := "bulk"]
    .dt[ct != "bulk", ct.vs.bulk := "sorted"]

    c1 <- function(cutoff){
        .ret <- .dt[wilcox.pv < cutoff,
                    .(n = unique(length(gene))),
                    by = .(ct, ct.vs.bulk)]
        .ret[, cutoff := cutoff]
        return(.ret)
    }
    ...
    c2 <- function(cutoff){
        .ret <- .dt[wilcox.pv < cutoff,
                    .(ntot = unique(length(gene))),
                    by = .(ct.vs.bulk)]
        .ret[, cutoff := cutoff]
        return(.ret)
    }

    ret.1 <- lapply(.cutoff, c1) %>% do.call(what=rbind)
    ret.2 <- lapply(.cutoff, c2) %>% do.call(what=rbind)
        
    ret <- merge(ret.1, ret.2)
    ret[, method := .method]
    return(ret)
}

count.tab <- 
    rbind(.count.discovery("cocoa"),
          .count.discovery("tot"),
          .count.discovery("avg"))
```


```{r Fig_DEG_ct_prop, fig.width=5, fig.height=2}
.dt <- count.tab[ct != "bulk"]

.col <- 
    left_join(tibble(ct = sort(unique(.dt$ct))), .colors) %>%
    select(hex) %>%
    unlist %>% 
    as.character
...
plt <-
    .gg.plot(.dt, aes(x = log10(cutoff), y = n/ntot, fill = ct)) +
    facet_wrap(~method, scales="free") +
    geom_bar(stat = "identity") +
    scale_fill_manual(values = .col) +
    scale_x_continuous("p-value cutoff", labels = function(x) 10^(x), breaks = .brk) +
    theme(legend.position = "bottom")

print(plt)

.file <- fig.dir %&% "/Fig_DEG_ct_prop.pdf"
.gg.save(.file, plot=plt, width = 5, height = 2)
```
...
```{r Fig_DEG_bulk_count, fig.width=5, fig.height=2}
.brk <- seq(-8, 0)

plt <- 
    .gg.plot(count.tab, aes(x = log10(cutoff), y = ntot, fill = ct.vs.bulk)) +
    facet_wrap(~method, scales="free") +
    geom_line() +
    geom_point(pch = 21) +
    scale_y_continuous("# unique genes", labels = function(x) num.int(x)) +
    scale_x_continuous("p-value cutoff", labels = function(x) 10^(x), breaks = .brk) +
...
.file <- fig.dir %&% "/Fig_DEG_bulk_count.pdf"
.gg.save(.file, plot=plt, width = 5, height = 2)
```
...
* `r num.int(n.fwer01.genes)` genes (`r num.round(100 * n.fwer01.genes / n.tot.genes)`%) consist of `r paste(n.fwer01.ct.genes$str, collapse=", ")`

* `r n.tot.genes` genes in total.
...
read.pheno <- function() {
    ...
    pheno.dt[, projid := as.integer(projid)]
...
#### Cell type information is not particularly associated with phenotypes

```{r Fig_CT_vs_AD, fig.width=5, fig.height=3}
pheno.dt <- read.pheno()
...
.ct.ass.dt <-
    ...
    left_join(pheno.dt[, .(projid, pathoAD)]) %>%
    mutate(pathoAD = factor(pathoAD, c(0, 1), c("non-AD", "AD"))) %>%
    mutate(ct = factor(ct, rev(.marker.order$ro))) %>%
    filter(!is.na(pathoAD))

plt <- 
    .gg.plot(.ct.ass.dt, aes(x = pathoAD, y = pr * 100)) +
    facet_wrap(~ct, scales="free", nrow = 2) +
    ylab("cell type fraction (%)") +
    xlab("disease label") +
    geom_boxplot(outlier.stroke = 0, outlier.size = 0, width = .2) +
    ggpubr::stat_compare_means(size = 2)

print(plt)

.file <- fig.dir %&% "/Fig_CT_vs_AD.pdf"
.gg.save(.file, plot=plt, width = 5, height = 3)
```
...
```{r Fig_cf_vs_avg_tstat, fig.width=8, fig.height=1.5, results="asis"}
plt <- 
...
```{r Fig_cf_vs_cocoa_tstat, fig.width=8, fig.height=1.5, results="asis"}
.dt.1 <- cocoa.dt[method == "cocoa", .(gene, ct, t.stat, method)]
.dt.2 <- .cf.assoc.stat[method == "cf", .(gene, ct, t.stat, method)]

.cocoa.cf.dt <-rbind(.dt.1, .dt.2)

.effect.dt <- dcast(.cocoa.cf.dt,
                    gene + ct ~ method, value.var = "t.stat",
                    fun.aggregate=mean, fill = NA) %>% 
    mutate(ct = factor(ct, rev(.marker.order$ro)))

plt <-
    .gg.plot(.effect.dt, aes(x = cf, y = cocoa, fill = ct)) +
    facet_wrap(~ct, scales="free", nrow = 1) +
    geom_point(pch = 21, stroke = .1) +
    geom_smooth(method="lm", se=FALSE, size=.2, formula=y~x, colour="black") +
    ggpubr::stat_cor(label.x.npc="left", label.y.npc="bottom", size=2) +
...
.file <- fig.dir %&% "/Fig_cf_vs_cocoa_tstat.pdf"
print(plt)
.gg.save(.file, plot=plt, width = 8, height = 2)
cat("\n\n[PDF](" %&% .file %&% ")\n\n")
```
...
```{r Fig_avg_vs_cf_tstat, fig.width=8, fig.height=1.5, results="asis"}
.dt.1 <- cocoa.dt[method == "cocoa", .(gene, ct, t.stat, method)]
.dt.2 <- avg.dt[method == "avg", .(gene, ct, t.stat, method)]

.cocoa.avg.dt <-rbind(.dt.1, .dt.2)

.effect.dt <- dcast(.cocoa.avg.dt,
                    gene + ct ~ method, value.var = "t.stat",
                    fun.aggregate=mean, fill = NA) %>% 
    mutate(ct = factor(ct, rev(.marker.order$ro)))

plt <-
    .gg.plot(.effect.dt, aes(x = avg, y = cocoa, fill = ct)) +
    facet_wrap(~ct, scales="free", nrow = 1) +
    geom_point(pch = 21, stroke = .1) +
    geom_smooth(method="lm", se=FALSE, size=.2, formula=y~x, colour="black") +
    ggpubr::stat_cor(label.x.npc="left", label.y.npc="bottom", size=2) +
...
.file <- fig.dir %&% "/Fig_avg_vs_cocoa_tstat.pdf"
print(plt)
.gg.save(.file, plot=plt, width = 8, height = 2)
cat("\n\n[PDF](" %&% .file %&% ")\n\n")
```
...
for(.ct in ct.names) {
    ...
    .top.genes <- select.genes(.ct, .01) %>% head(5)

    if(length(.top.genes) >= 1) {
        p.list <-
            lapply(.top.genes,
                   function(g) {
...
measure.size <- function(go.ct.dt, fdr.cutoff) {
    .nrow <- length(unique(go.ct.dt[qv < fdr.cutoff & n.w.drawn > 2]$name_1006))
    .ncol <- length(unique(go.ct.dt[qv < fdr.cutoff & n.w.drawn > 2]$ct))
    .lab <- max(sapply(go.ct.dt[qv < fdr.cutoff & n.w.drawn > 2]$name_1006, nchar))

    list(w = (.lab * .055 + .ncol * .05 + .5),
         h = ceiling(.075 * .nrow + .25))
}
```


#### DEGs up-regulated (`r num.int(nrow(unique(cocoa.dt[qv < .05 & t.stat > 0, .(gene)])))` genes)

```{r Fig_GO_Enrichment_CT_up, fig.width=5.695, fig.height=5}
go.ct.dt <-
...
#### A table for the GO keywords enriched by the up-regulated genes

```{r include = FALSE}
dir.create("Tab/AD/", recursive=TRUE, showWarnings=FALSE)
out.dt <- go.ct.dt[qv < .1]
fwrite(out.dt, file="Tab/AD/GO_CT_UP.txt.gz", row.names=FALSE, col.names=TRUE, sep="\t")
```


#### DEGs down-regulated (`r num.int(nrow(unique(cocoa.dt[qv < .05 & t.stat < 0, .(gene)])))` genes)

```{r Fig_GO_Enrichment_CT_down, fig.width=3.405, fig.height=1}
go.ct.dt <-
    lapply(ct.names, function(.ct) {
        test.go.enrichment(cocoa.dt,
                           gene.ontology$go,
...
#### A table for the GO keywords enriched by the down-regulated genes

```{r include = FALSE}
dir.create("Tab/AD/", recursive=TRUE, showWarnings=FALSE)
out.dt <- go.ct.dt[qv < .1]
fwrite(out.dt, file="Tab/AD/GO_CT_DOWN.txt.gz", row.names=FALSE, col.names=TRUE, sep="\t")
```

...
.pheno <- fread("data/brain_2018-05-03/filtered_column_metadata.txt.gz")
...
* `r num.int(round(mean(.tot.i.freq.dt$n)))` cells per individual, including `r paste(.avg.ct.dt$str, collapse=", ")` on average.
...
show.gene.comparison(local.dt[gene == "APOE"])
```
...
show.one.gene(local.dt[gene == "APOE"], cocoa.dt[gene == "APOE" & ct == "Microglia"])
```
...
```{r include=FALSE}
test.go.enrichment <- function(.dt, .go.tab, fdr.cutoff = NULL, ntop = NULL) {
    ...
    ndrawn <- .temp[pv < pv.cutoff] %>% nrow

    .merged <- merge(.temp, .go.tab)

    ret <-
        .merged[, .(n.white = .N,
                   n.black = ntot - .N,
                   n.w.drawn = sum(pv < pv.cutoff),
                   n.drawn = ndrawn),

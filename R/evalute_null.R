...
efdr <- function(target.fdr, .stat) {
    .found <- .stat[q < target.fdr, .(x.col)] %>% nrow
    .tot <- nrow(.stat)
    data.table(target = target.fdr, efdr = .found / .tot)
}

marg.dt <- .read.named(hdr, ".ln_obs_mu.gz")
cocoa.dt <- .read.named(hdr, ".ln_resid_mu.gz")

target <- c(0.2, 0.1, 0.05, 0.01)

cocoa.stat <-
    lapply(target, efdr, .stat = calc.stat(cocoa.dt, FALSE)) %>%
    do.call(what=rbind) %>%
    mutate(method = "cocoa")

marg.stat <-
    lapply(target, efdr, .stat = calc.stat(marg.dt, FALSE)) %>%
    do.call(what=rbind) %>%
    mutate(method = "marg")

out.stat <- rbind(cocoa.stat, marg.stat) %>%
    mutate(sim = hdr) %>%
